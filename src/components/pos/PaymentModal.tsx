...
interface PaymentModalProps {
  ...
  onClose: () => void;
  totalDue: number;
  onProcessPayment?: (payments: PaymentMethod[]) => Promise<void>;
}

export const PaymentModal = ({ isOpen, onClose, totalDue, onProcessPayment }: PaymentModalProps) => {
  ...
  const addPayment = (type: PaymentMethod['type']) => {
    if (amountToApply <= 0 && type !== 'CASH') return; // Allow cash overpayment for change
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Process Payment" size="lg">
      <div className="flex flex-col h-[500px]">
        ...
        <div className="border-t border-slate-100 pt-6 mt-auto flex justify-end gap-3">
          ...
          >
            {isProcessing ? 'Processing...' : 'Finalize Receipt'}
          </Button>
        </div>
      </div>
    </Modal>
