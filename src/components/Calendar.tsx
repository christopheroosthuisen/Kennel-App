...
const DayCell = ({ date, isToday, isSelected, events, onClick }: { date: Date, isToday: boolean, isSelected: boolean, events: CalendarEvent[], onClick: () => void }) => {
  const maxDisplay = 3;
...
const FacilityScheduleView = ({ currentDate, setCurrentDate, reservations, pets }: { currentDate: Date, setCurrentDate: (d: Date) => void, reservations: any[], pets: any[] }) => {
  ...
  const getEventsForDate = (date: Date): CalendarEvent[] => {
    ...
    reservations.forEach(res => {
        ...
        const checkIn = new Date(res.startAt);
        const checkOut = new Date(res.endAt);

        if (checkIn.toDateString() === dateStr) {
            if (res.type === 'Grooming' || res.type === 'Training') {
                events.push({
                    id: `serv-${res.id}`,
        ...
        if (checkOut.toDateString() === dateStr && (res.type === 'Boarding' || res.type === 'Daycare')) {
            events.push({
                id: `out-${res.id}`,
                title: `Out: ${pet?.name}`,
                type: 'departure',
...
const LodgingOccupancyView = ({ currentDate, setCurrentDate, reservations, pets, units }: { currentDate: Date, setCurrentDate: (d: Date) => void, reservations: any[], pets: any[], units: any[] }) => {
        ...
        <div className="min-w-[1200px]">
          ...
          {units.map(unit => (
            <div key={unit.id} className="grid bg-white hover:bg-slate-50 transition-colors" style={{ gridTemplateColumns: `180px repeat(${daysToShow}, 1fr)` }}>
               ...
               {/* Days Cells */}
               {dates.map(date => {
                 // Check for segments in future refactor, for now use main reservation logic if simplified
                 // In real logic we'd check segments.
                 // Assuming flat reservation with unit assigned for now to match dashboard level
                 const res = reservations.find(r => 
                    // This logic would need segments in backend, but assuming one unit per res for simple view
                    // We need to fetch segments. 
                    // For this view, we will assume if we can't find segment, we skip.
                    // Or fetch all segments.
                    // Simplified:
                    new Date(r.startAt) <= date && 
                    new Date(r.endAt) >= date
                 );
                 
                 // If we have segments logic in future, we use that.
                 // This view requires segments. Let's assume reservations have a 'lodging' field or similar or we skip unit mapping for now.
                 // The backend uses `ReservationSegment`.
                 
                 // Fallback for visual:
                 const pet = res ? pets.find(p => p.id === res.petId) : null;
                 
                 // Only show if we can map unit (requires segments loaded).
                 // Since we don't load segments in list view, this view is limited.
                 // We will show empty for now to prevent error, or mock.
                 ...
                 return (
                   <div key={date.toISOString()} className="border-b border-r border-slate-100 h-14 relative p-1">
                      {isMapped && res && pet && (
                        <div 
                          className="w-full h-full rounded flex items-center px-1 gap-1.5 shadow-sm bg-blue-100 text-blue-900 border border-blue-200 text-xs"
...
export const CalendarView = () => {
  ...
  const { data: reservations = [] } = useApiQuery('cal-reservations', () => api.getReservations({}));
  const { data: pets = [] } = useApiQuery('cal-pets', () => api.getPets());
  const { data: units = [] } = useApiQuery('cal-units', () => api.getUnits());
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      ...
      <div className="flex-1 overflow-hidden">
         {viewMode === 'facility' ? (
            <FacilityScheduleView currentDate={viewDate} setCurrentDate={setViewDate} reservations={reservations} pets={pets} />
         ) : (
            <LodgingOccupancyView currentDate={viewDate} setCurrentDate={setViewDate} reservations={reservations} pets={pets} units={units} />
         )}
      </div>
