...
export const CareDashboard = () => {
  const { careTasks, pets, updateCareTask } = useData();
  ...
  // Derived State
  const filteredTasks = careTasks.filter(t => t.shift === currentShift && (activeTab === 'feeding' ? t.type === 'Feeding' : t.type === 'Medication'));
  ...
  const handleStatusChange = (taskId: string, newStatus: CareTask['status']) => {
     updateCareTask(taskId, { status: newStatus, completedAt: newStatus === 'Completed' ? new Date().toISOString() : undefined });
  };

  const handleBatchAction = (status: CareTask['status']) => {
     filteredTasks.filter(t => t.status !== 'Completed').forEach(t => {
        updateCareTask(t.id, { status });
     });
  };
          ...
          <div className="flex gap-4">
             <div className="flex">
                <button 
                   ...
                   className={cn("flex items-center gap-2 px-4 py-2 border-b-2 transition-colors", activeTab === 'feeding' ? "border-primary-600 text-primary-700 bg-primary-50" : "border-transparent text-slate-500 hover:bg-slate-50")}
                >
                   <Utensils size={16}/> Feeding
                </button>
                <button 
                   onClick={() => setActiveTab('medication')}
                   className={cn("flex items-center gap-2 px-4 py-2 border-b-2 transition-colors", activeTab === 'medication' ? "border-red-500 text-red-700 bg-red-50" : "border-transparent text-slate-500 hover:bg-slate-50")}
                >
             ...
             <div className="flex items-center gap-2 px-2">
                <button onClick={() => setViewMode('prep')} className={cn("p-1.5 rounded", viewMode === 'prep' ? "bg-slate-100 text-slate-900" : "text-slate-400 hover:text-slate-600")} title="Prep View"><LayoutGrid size={18}/></button>
                <button onClick={() => setViewMode('distribution')} className={cn("p-1.5 rounded", viewMode === 'distribution' ? "bg-slate-100 text-slate-900" : "text-slate-400 hover:text-slate-600")} title="Distribution View"><List size={18}/></button>
             </div>
          </div>
       ...
       <div className="flex-1 overflow-y-auto">
          {viewMode === 'prep' ? (
                ...
                {['Pending', 'Prepared', 'Completed'].map(statusGroup => {
                   ...
                   return (
                      <div key={statusGroup} className="flex flex-col gap-3">
                         ...
                         {groupTasks.map(task => <CareCard key={task.id} task={task} onStatusChange={handleStatusChange} viewMode="prep" pets={pets}/>)}
                         {groupTasks.length === 0 && <div className="p-8 text-center border-2 border-dashed border-slate-100 rounded-lg text-slate-300 text-sm">No {statusGroup.toLowerCase()} tasks</div>}
                      </div>
                   )
                })}
             </div>
             ...
             <div className="space-y-2 max-w-4xl mx-auto">
                {filteredTasks.sort((a,b) => a.unit.localeCompare(b.unit)).map(task => <CareCard key={task.id} task={task} onStatusChange={handleStatusChange} viewMode="distribution" pets={pets}/>)}
             </div>
          )}
       </div>
    </div>
...
const CareCard: React.FC<CareCardProps> = ({ task, onStatusChange, viewMode, pets }) => {
      ...
      return (
         <div className={cn("bg-white border border-slate-200 rounded-lg p-3 flex items-center justify-between shadow-sm hover:shadow-md transition-all", borderClass, task.status === 'Completed' && "opacity-60 bg-slate-50")}>
            <div className="flex items-center gap-4">
               ...
               <div className="h-10 w-10 bg-slate-100 rounded-full overflow-hidden"><img src={pet?.photoUrl} className="w-full h-full object-cover" alt="" /></div>
               <div>
                  <div className="font-bold text-slate-900 flex items-center gap-2">{pet?.name} {task.warning && <AlertTriangle size={14} className="text-red-500"/>}</div>
            ...
            <div className="flex items-center gap-4">
               ...
               {task.status === 'Completed' ? <div className="flex items-center gap-1 text-green-600 font-bold text-sm"><CheckCircle size={16}/> Done</div> : <Button size="sm" className="bg-green-600 hover:bg-green-700" onClick={() => onStatusChange(task.id, 'Completed')}>Mark Given</Button>}
            </div>
         </div>
      )
   }
   ...
   return (
      <Card className={cn("p-4 transition-all hover:shadow-md", borderClass, task.status === 'Completed' && "opacity-60")}>
         ...
         <div className="flex gap-2">
            {task.status === 'Pending' && <Button size="sm" variant="outline" className="flex-1 border-primary-200 text-primary-700 hover:bg-primary-50" onClick={() => onStatusChange(task.id, 'Prepared')}>Mark Prepared</Button>}
            {task.status === 'Prepared' && <Button size="sm" className="flex-1 bg-green-600 hover:bg-green-700" onClick={() => onStatusChange(task.id, 'Completed')}>Mark Distributed</Button>}
            {task.status === 'Completed' && <Button size="sm" variant="ghost" className="flex-1 text-slate-400" onClick={() => onStatusChange(task.id, 'Pending')}>Undo</Button>}
         </div>
      </Card>
   );
};
